...
const LEGACY_LOCAL_TOKEN_PREFIX = "local:";
const LEGACY_LOCAL_KEYS = ["cm_local_users", "cm_local_history"];
...
const clearLegacyFallbackState = () => {
...
export const getToken = () => {
  ...
  if (String(token || "").startsWith(LEGACY_LOCAL_TOKEN_PREFIX)) {
    localStorage.removeItem(TOKEN_KEY);
    clearLegacyFallbackState();
    return null;
  }
  return token;
};
...
export const savePreferences = (data) =>
  apiRequest("/user/preferences", { method: "POST", body: data });
...
export const addWatchHistory = (data) =>
  apiRequest("/user/history", { method: "POST", body: data });

export const removeWatchHistory = (movieId) =>
  apiRequest(`/user/history/${movieId}`, { method: "DELETE" });

export const toggleFavoriteMovie = (movieId) =>
  apiRequest("/user/favorite", { method: "POST", body: { movieId } });
